...
export const KnowledgesListConfig: React.FC = () => {
  const { knowledgesListConfig, updateKnowledgesListConfig } = useWidgetConfig();
        ...
        <div className="space-y-3">
          <div>
            <Input
              ...
              id="widget-title"
              value={title}
              onChange={(value) => updateKnowledgesListConfig({ title: value })}
          ...
          <div>
            <TextArea
              ...
              id="description"
              value={description}
              onChange={(e) => updateKnowledgesListConfig({ description: e.target.value })}
            ...
            <div className="grid grid-cols-3 gap-2">
              {styleOptions.map((option) => (
                <StyleTile
                  ...
                  onClick={() => updateKnowledgesListConfig({ style: option.id as 'card' | 'list' | 'feed' })}
                />
              ))}
            </div>
          </div>
            ...
            <>
              <div>
                <Select 
                  ...
                  onSelectionChange={(key) => updateKnowledgesListConfig({ cardSize: key as 'small' | 'medium' | 'large' | 'extralarge' })}
                >
              ...
              <div>
                <Select 
                  ...
                  onSelectionChange={(key) => updateKnowledgesListConfig({ cardStyle: key as 'modern' | 'simple' })}
                >
                  {(item) => <Select.Item id={item.id} label={item.label} icon={item.icon} />}
                </Select>
              </div>
        ...
        <div className="space-y-2">
          {!(style === 'card' && cardStyle === 'modern') && (
            <PropertyToggle
              ...
              onChange={(value) => updateKnowledgesListConfig({ coverImage: value })}
              id="cover-image"
            />
          )}
          <PropertyToggle
            ...
            onChange={(value) => updateKnowledgesListConfig({ authorInfo: value })}
            id="author-info"
          />
          ...
          <PropertyToggle
            ...
            onChange={(value) => updateKnowledgesListConfig({ reactionsCounter: value })}
          ...
          <PropertyToggle
            ...
            label="Read time"
            isSelected={readTime}
            onChange={(value) => updateKnowledgesListConfig({ readTime: value })}
            id="read-time"
          />
        </div>
      </CustomizerSection>
