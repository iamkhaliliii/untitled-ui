...
export const QuestionsListConfig: React.FC = () => {
  const { questionsListConfig, updateQuestionsListConfig } = useWidgetConfig();
        ...
        <div className="space-y-3">
          <div>
            <Input
              ...
              id="widget-title"
              value={title}
              onChange={(value) => updateQuestionsListConfig({ title: value })}
          ...
          <div>
            <TextArea
              ...
              id="description"
              value={description}
              onChange={(e) => updateQuestionsListConfig({ description: e.target.value })}
            ...
            <div className="grid grid-cols-3 gap-2">
              {styleOptions.map((option) => (
                <StyleTile
                  ...
                  onClick={() => updateQuestionsListConfig({ style: option.id as 'card' | 'list' | 'feed' })}
                />
              ))}
            </div>
          </div>
            ...
            <>
              <div>
                <Select 
                  ...
                  onSelectionChange={(key) => updateQuestionsListConfig({ cardSize: key as 'small' | 'medium' | 'large' | 'extralarge' })}
                >
              ...
              <div>
                <Select 
                  ...
                  onSelectionChange={(key) => updateQuestionsListConfig({ cardStyle: key as 'modern' | 'simple' })}
                >
                  {(item) => <Select.Item id={item.id} label={item.label} icon={item.icon} />}
                </Select>
              </div>
      ...
      >
        <div className="space-y-2">
          <PropertyToggle
            ...
            onChange={(value) => updateQuestionsListConfig({ authorInfo: value })}
            id="author-info"
          />
          ...
          <PropertyToggle
            ...
            isSelected={answersCounter}
            onChange={(value) => updateQuestionsListConfig({ answersCounter: value })}
            id="answers-counter"
          />
          ...
          <PropertyToggle
            ...
            onChange={(value) => updateQuestionsListConfig({ votesCounter: value })}
            id="votes-counter"
          />
