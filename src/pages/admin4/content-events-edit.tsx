...
export const AdminContentEventsEditPage = () => {
    ...
    const eventId = params.id || 'sample-event-id';
    
    // Mock: In a real app, you'd fetch the event data based on the eventId
    const [formData, setFormData] = useState<FormData>({
        // Step 1: Event Details - Pre-populated with existing data
        title: 'Annual Tech Conference 2024',
        aboutEvent: '<p>Join us for our biggest tech event of the year!</p>',
        space: 'events',
        ...
        virtualUrls: [{ url: 'zoom.us/j/123456789', id: 'virtual-url-1' }],
        coverImage: null,
        eventDetails: 'Bring your laptop and notebook.',
        additionalDetails: 'Building B, 3rd Floor, Conference Room A',
        
        // Recurring Event Settings - DEFAULT: Recurring Event Enabled
        isRecurring: true,
        ...
        metaTitle: 'Annual Tech Conference 2024',
        metaDescription: 'Join us for our biggest tech event of the year with amazing speakers and workshops.',
    ...
    // Popover and modal states for recurring event updates
    ...
    const [showDateEditWarning, setShowDateEditWarning] = useState(false);
    const [updateScope, setUpdateScope] = useState<'this' | 'following' | 'all'>('this');
    const [pendingFormData, setPendingFormData] = useState<FormData | null>(null);
    const updateButtonRef = useRef<HTMLDivElement>(null);
    const [userAcknowledgedDateChange, setUserAcknowledgedDateChange] = useState(false);
    ...
    const datesHaveChanged = useMemo(() => {
        if (!formData.dateFrom || !formData.dateTo || !originalDates.dateFrom || !originalDates.dateTo) return false;
        
        const dateFromChanged = 
            formData.dateFrom.year !== originalDates.dateFrom.year ||
            formData.dateFrom.month !== originalDates.dateFrom.month ||
            formData.dateFrom.day !== originalDates.dateFrom.day;
        
        const dateToChanged = 
            formData.dateTo.year !== originalDates.dateTo.year ||
            formData.dateTo.month !== originalDates.dateTo.month ||
            formData.dateTo.day !== originalDates.dateTo.day;
        
        const timeFromChanged = 
            formData.timeFrom?.hour !== originalDates.timeFrom?.hour ||
            formData.timeFrom?.minute !== originalDates.timeFrom?.minute;
        
        const timeToChanged = 
            formData.timeTo?.hour !== originalDates.timeTo?.hour ||
            formData.timeTo?.minute !== originalDates.timeTo?.minute;
        
        return dateFromChanged || dateToChanged || timeFromChanged || timeToChanged;
    }, [formData, originalDates]);

    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        
        // If this is a recurring event, show the update popover
        ...
        // If not recurring, proceed with normal update
        await performUpdate(formData, 'this');
    };
    
    const performUpdate = async (data: FormData, scope: 'this' | 'following' | 'all') => {
        setIsSubmitting(true);
        try {
            console.log("Updating event data:", data);
            console.log("Update scope:", scope);
            // Here you would typically send the data to your API
            // await updateEvent(eventId, data, scope);
    ...
    const handlePopoverUpdate = (scope: 'this' | 'following' | 'all') => {
        setUpdateScope(scope);
        setShowUpdatePopover(false);
        
        // If dates have changed, show warning about RSVP list
        if (datesHaveChanged) {
            setShowDateChangeWarning(true);
        } else {
            // Proceed with update
            if (pendingFormData) {
                performUpdate(pendingFormData, scope);
            }
        }
    };
    
    const handleDateChangeConfirm = () => {
        // User confirmed they understand RSVPs will be lost
        if (pendingFormData) {
            performUpdate(pendingFormData, updateScope);
        }
    };
    
    const handleDateFieldClick = () => {
        // Show warning when user tries to edit date/time fields in recurring event
        if (formData.isRecurring && !userAcknowledgedDateChange) {
            setShowDateEditWarning(true);
        }
    };
    
    const handleDateEditAcknowledge = () => {
        setUserAcknowledgedDateChange(true);
        setShowDateEditWarning(false);
    };
    
    // Close popover when clicking outside
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (updateButtonRef.current && !updateButtonRef.current.contains(event.target as Node)) {
                setShowUpdatePopover(false);
            }
        };
    ...
    return (
        <div className="w-full flex flex-col max-w-full self-center space-y-0 sm:space-y-0 md:space-y-0 lg:space-y-0 bg-background dark:bg-gray-950 h-screen transition duration-200 ms-[calc(env(safe-area-inset-left))] me-[calc(env(safe-area-inset-right))] overflow-hidden">
                        ...
                        <div className="px-4 py-5 sm:p-6 pb-0 sm:pb-0">
                            <h3 className="text-content dark:text-gray-100 font-medium text-heading-xs">
                                <div className="flex space-x-3 items-center">
                                    ...
                                    <span className="text-xl font-medium flex-1 text-gray-900 dark:text-gray-100">Edit event</span>
                                </div>
                            </h3>
                        </div>
                                ...
                                <div className="w-full flex flex-col max-w-full md:max-w-3xl self-center space-y-6">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        <div>
                                            ...
                                            <div 
                                                className={`flex gap-0 mt-1 border border-gray-300 rounded-lg overflow-hidden dark:border-gray-700 ${formData.isRecurring && !userAcknowledgedDateChange ? 'cursor-pointer' : ''}`}
                                                onClick={handleDateFieldClick}
                                            >
                                                <div className={formData.isRecurring && !userAcknowledgedDateChange ? 'pointer-events-none opacity-60' : ''}>
                                                    <DatePicker
                                    ...
                                    {/* Recurring Event Info - Show as alert/notice */}
                                    {formData.isRecurring && recurringPatternDescription && (
                                        <div className="flex items-start gap-3 p-4 rounded-xl bg-brand-50 dark:bg-brand-900/20 border border-brand-200 dark:border-brand-800">
                                            <Repeat03 className="w-5 h-5 text-brand-600 dark:text-brand-400 flex-shrink-0 mt-0.5" />
                                            <div className="flex-1">
                                                <div className="flex items-center gap-2 mb-1">
                                                    <span className="text-sm font-semibold text-brand-700 dark:text-brand-300">Recurring Event</span>
                                                </div>
                                    ...
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-3 relative" ref={updateButtonRef}>
                                            <Button
                                                size="sm"
                                                color="primary"
                                                type="submit"
                                            ...
                                            {/* Update Scope Popover - Shows next to button */}
                                            {showUpdatePopover && (
                                                <>
                                                    {/* Backdrop */}
                                                    <div 
                                                    ...
                                                    />
                                                    {/* Popover */}
                                                    <div className="absolute bottom-full right-0 mb-2 z-50 bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 p-4 w-80">
                                                        <div className="mb-3 pb-3 border-b border-gray-100 dark:border-gray-700">
                                                            <div className="flex items-center gap-2 mb-1">
                                                                <Calendar className="h-5 w-5 text-brand-solid" />
                                                                <h4 className="text-sm font-semibold text-gray-900 dark:text-gray-100">Update recurring event</h4>
                                                            </div>
                                                            <p className="text-xs text-gray-600 dark:text-gray-400 text-left">Which events would you like to update?</p>
                                                        </div>
                                                        <div className="space-y-2">
                                                            {/* This event only */}
                                                            <div 
                                                                className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-colors border border-transparent hover:border-gray-200 dark:hover:border-gray-700"
                                                                onClick={() => handlePopoverUpdate('this')}
                                                            >
                                                                <Calendar className="h-4 w-4 text-gray-500 dark:text-gray-400 mt-0.5 flex-shrink-0" />
                                                                <div className="flex-1 text-left">
                                                                    <div className="text-sm font-medium text-gray-900 dark:text-gray-100">This event</div>
                                                                    <div className="text-xs text-gray-600 dark:text-gray-400">Only this event will be updated</div>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div 
                                                                className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-colors border border-transparent hover:border-gray-200 dark:hover:border-gray-700"
                                                                onClick={() => handlePopoverUpdate('following')}
                                                            >
                                                                <Calendar className="h-4 w-4 text-gray-500 dark:text-gray-400 mt-0.5 flex-shrink-0" />
                                                                <div className="flex-1 text-left">
                                                                    <div className="text-sm font-medium text-gray-900 dark:text-gray-100">This and following events</div>
                                                                    <div className="text-xs text-gray-600 dark:text-gray-400">This event and all future events</div>
                                                                </div>
                                                            </div>
                                                            ...
                                                            <div 
                                                                className="flex items-start gap-3 p-3 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-colors border border-transparent hover:border-gray-200 dark:hover:border-gray-700"
                                                                onClick={() => handlePopoverUpdate('all')}
                                                            >
                                                                <Repeat03 className="h-4 w-4 text-gray-500 dark:text-gray-400 mt-0.5 flex-shrink-0" />
                                                                <div className="flex-1 text-left">
                                                                    <div className="text-sm font-medium text-gray-900 dark:text-gray-100">All events</div>
                                                                    <div className="text-xs text-gray-600 dark:text-gray-400">All events in this series</div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </div>
            ...
            {/* Date Edit Warning Modal - Shows when user tries to edit date fields */}
            {showDateEditWarning && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/70 backdrop-blur-sm">
                    <div 
                        className="absolute inset-0"
                        onClick={() => setShowDateEditWarning(false)}
                    />
                    <div 
                        className="relative bg-white dark:bg-gray-900 rounded-xl shadow-xl max-w-2xl w-full p-6"
                    ...
                    >
                        <div className="space-y-4">
                            ...
                            <div className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
                                <p>This event repeats on a schedule.</p>
                                <p>Changing its date or time will recreate all events in the series.</p>
                            </div>
                            ...
                            <div className="border-t border-gray-200 dark:border-gray-700 pt-4 pb-2">
                                ...
                                <ul className="space-y-1.5 text-gray-700 dark:text-gray-300 text-sm">
                                    <li className="flex items-start gap-2">
                                        ...
                                        <span>The current RSVP list will be cleared</span>
                                    </li>
                                    <li className="flex items-start gap-2">
                                        <span className="text-gray-400 dark:text-gray-500 mt-0.5">•</span>
                                        <span>Attendees will need to RSVP again</span>
                                    </li>
                                    <li className="flex items-start gap-2">
                                        <span className="text-gray-400 dark:text-gray-500 mt-0.5">•</span>
                                        <span>Any event-specific data will be reset</span>
                                    </li>
                                </ul>
                            </div>
                            
                            <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                                <p className="font-medium text-gray-900 dark:text-gray-100 text-sm mb-4">
                                ...
                                <div className="flex items-center justify-end gap-3">
                                    ...
                                    <Button
                                        size="sm"
                                        color="primary"
                                        onClick={handleDateEditAcknowledge}
                                    >
                                        Yes, continue
                                    </Button>
                                </div>
            ...
            {/* Date Change Warning Modal - Shows after scope selection if dates changed */}
                            ...
                            <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                                ...
                                <div className="flex items-center justify-end gap-3">
                                    ...
                                    <Button
                                        ...
                                        disabled={isSubmitting}
                                    >
                                        {isSubmitting ? 'Updating...' : 'Yes, continue'}
                                    </Button>
                                </div>
                            </div>
                        </div>
